...
class DenonDevice(object):
    ...
    def source(self):
        ...
        media_title = self.telnet_request(
            'get_now_playing_media?pid={0}'.format(self.pid))
    ...
    def media_play(self):
        ...
        self.telnet_command(
            'set_play_state?pid={0}&state=play'.format(self.pid))

    def media_pause(self):
        """Pause media player."""
        self.telnet_command(
            'set_play_state?pid={0}&state=pause'.format(self.pid))

    def media_stop(self):
        """Stop media media player."""
        self.telnet_command(
            'set_play_state?pid={0}&state=stop'.format(self.pid))
